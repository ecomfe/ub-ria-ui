...
define(
    function (require) {
            ...
            if (header) {
                ...
                if (content) {
                    ...
                    if (accordion.fixHeight) {
                        content.style.cssText = 'height  : ' + parseInt(accordion.fixHeight, 10) + 'px;' +
                                                'overflow: auto';
                    }
                }
            }
         ...
         * @type {string}
         */
        Accordion.prototype.panelTemplate = '<div data-role="header">' +
                                                    '${header}' +
                                                    '<i></i>' +
                                              '</div>' +
            ...
            if (this.helper.isPart(target, 'header')) {
                ...
                if (this.collapsible) {
                    ...
                    if (this.helper.isPart(panel, 'panel-active')) {
                        // 触发折叠事件
                        this.fire('collapse', {collapse: true});
                        // 标记为所有元素折叠
                        this.activeIndex = -1;
                    }
                    else {
        ...
        function activateAccordion(accordion, index) {
            ...
            for (var i = 0; i < accordion.panels.length; i++) {
                ...
                if (config.content) {
                    config.content.style.display = i === index ? '' : 'none';
                }
         ...
         *
         */
        Accordion.prototype.oncollapse = function (config) {
            var panel = this.panels[this.activeIndex];

            if (panel.content) {
                panel.content.style.display = config.collapse ? 'none' : '';
            }
            var panelEl = lib.getChildren(this.main)[this.activeIndex];
            var methodName =
                config.collapse ? 'removePartClasses' : 'addPartClasses';
            this.helper[methodName]('panel-active', panelEl);
        };
        ...
        Accordion.prototype.removeAt = function (index) {
            ...
            if (removed) {
                ...
                // 隐藏对应的元素
                if (removed.content) {
                    removed.content.style.display = 'none';
                }
            }
        };
