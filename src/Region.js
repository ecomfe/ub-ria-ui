...
define(
    function (require) {
         ...
         * @return {HTMLElement}
         */
        function getOptionCheckbox(region, id) {
            return lib.g(helper.getId(region, 'wrapper-' + id));
        }
        ...
        function updateSelectedTip(region, selCityIdsLength, cLength, id) {
            ...
            var checkboxDom = getOptionCheckbox(region, id);
            if (selCityIdsLength !== 0 && selCityIdsLength !== cLength) {
                ...
                lib.setAttribute(getOptionDOM(region, id), 'indeterminate', true);
                helper.addPartClasses(region, 'multi-custom', checkboxDom);
            }
            else {
                lib.addClass(infoTag, getHiddenClassName());
                infoTag.innerHTML = '';
                lib.setAttribute(getOptionDOM(region, id), 'indeterminate', false);
                helper.removePartClasses(region, 'multi-custom', checkboxDom);
            }
        }
         ...
         *  add by jiangyuan
         *  批量添加多个样式
         *  @param  {Region} region 控件实例.
         *  @param  {Option} className 样式名称.
         *  @return {String} classes   格式化后的样式字符串
         * */

        function getPartClassesGroup(region) {
            var args = [].slice.call(arguments, 1);
            var classGroup = [];
            if(args.length){
                u.each(args, function(className){
                    classGroup = u.union(classGroup, helper.getPartClasses(region, className));
                })
            }else{
                classGroup = helper.getPartClasses();
            }
            ...
            switch (level) {
                ...
                case 1:
                    return lib.format(
                        tplBoxWrapper,
                        {
                            ...
                            itemClasses: getPartClassesGroup(region, 'country-check', 'checkbox-custom'),
                                //helper.getPartClasses(
                ...
                case 2:
                    return lib.format(
                        tplBoxWrapper,
                        {
                            ...
                            itemClasses: getPartClassesGroup(region, 'region-check', 'checkbox-custom'),
                                //helper.getPartClasses(
                                //    region, 'region-check'
                                //).join(' '),
                            itemWrapperId: '',
                ...
                case 3:
                    var layer = lib.format(
                        tplPopLayer,
                        {
                            popLayerClass: getPartClassesGroup(region, 'locator'),
                                //helper.getPartClasses(
                                //    region,
                                //    'locator'
                                //).join(' '),
                            layerBoxClass: getPartClassesGroup(region, 'city-box'),
                                //helper.getPartClasses(
                                //    region,
                                //    'city-box'
                                //).join(' '),
                    ...
                    var text = lib.format(
                        tplInputItem,
                        {
                            itemClasses: getPartClassesGroup(region, 'text', 'checkbox-custom'),
                                //helper.getPartClasses(
                                //    region, 'text'
                                //).join(' '),
                            itemWrapperId:
                ...
                case 4:
                    return lib.format(
                        tplInputItem,
                        {
                            itemClasses: getPartClassesGroup(region, 'city', 'checkbox-custom'),
                                //helper.getPartClasses(
                                //    region, 'city'
                                //).join(' '),
                            itemWrapperId: '',
